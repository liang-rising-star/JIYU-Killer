...
本项目提供多种发布版本，请根据您的需求选择合适的版本：

| 版本名称 | 体积 | 需要 .NET 运行时 | 适用场景 |
| -------- | ---- | ---------------- | -------- |
| **Portable（便携版）** | ~75MB | ❌ 不需要 | 适合裸机运行，插U盘就能用 |
| **Framework（框架版）** | ~500KB | ✅ 需要 .NET 8 | 体积小巧，适合已安装 .NET 的电脑 |
| **Setup（安装版）** | ~75MB | ❌ 不需要 | 适合需要安装向导的用户 |
...
| 特性 | Portable 便携版 | Framework 框架版 | Setup 安装版 |
| ---- | --------------- | ---------------- | ------------ |
| **体积** | ~75MB | ~500KB | ~75MB |
| **.NET 运行时** | 内置，无需安装 | 需要额外安装 .NET 8 | 内置，无需安装 |
| **使用方法** | 双击直接运行 | 双击运行（需先安装 .NET） | 运行安装程序 |
| **适合场景** | 临时使用、U 盘携带 | 开发测试、空间有限 | 长期使用、一键安装 |
| **卸载方式** | 删除文件即可 | 删除文件即可 | 通过控制面板卸载 |

### 7.2 版本选择建议

- **便携版 (Portable)**：推荐大多数用户使用，无需安装任何依赖，下载即可运行
- **框架版 (Framework)**：如果你需要在多台电脑使用且电脑已安装 .NET 8，选择此版本可以节省磁盘空间
- **安装版 (Setup)**：如果你不熟悉电脑操作，想要像安装普通软件一样安装程序，选择此版本

### 7.3 下载方式

前往 [Release 页面](https://github.com/JIYU-Killer/JIYU-Killer/releases) 下载对应版本。
